...
# add apikey to authenticator
authenticator=IAMAuthenticator('B5VnQVGISdIn0czoSYPtCMqrVMUTbMYvaIXNUQ4XVzmV')
# tts mean text ot speach,  add authentiactor to TextToSpeechV1
tts =  TextToSpeechV1(authenticator=authenticator)
# my Url for text to speech in ibm watson
tts.set_service_url('https://api.us-south.text-to-speech.watson.cloud.ibm.com/instances/9fbe87ee-8c28-4c4b-96b2-3f8012bfdc72')
...
# append what user say to mp3 file, also append to txt file 
def TextMp3(user):
     with open ('user.mp3','ab') as file:
      file.write((tts.synthesize(user, accept='audio/mp3', voice='en-US_AllisonV3Voice').get_result() ).content)      
      newa = open("user.txt", "a")
      newa.write(user+"\n")
      newa.flush()
      newa.close
...
#convert text to audio and write it in mp3 file, then play mp3 file    
with open('hello.mp3',"wb") as file:        
     file.write((tts.synthesize('Hello How i can help you', accept='audio/mp3', voice='en-US_AllisonV3Voice').get_result() ).content)
...
def on_message(self, msg):
    ...
    if "results" in data:
        if data["results"][0]["final"]:
            FINALS.append(data)
            LAST = None
            user = ((data['results'][0]['alternatives'][0]['transcript']))
            # call TextMp3 function and send user to it
            TextMp3(user)
